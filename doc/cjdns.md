# CJDNS support in Quiquitocoin Core

It is possible to run Quiquitocoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Quiquitocoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Quiquitocoin network and individual nodes.
...
before it will work with your Quiquitocoin Core node. This is described in steps
...
## Run Quiquitocoin Core with CJDNS

Once you are connected to the CJDNS network, the following Quiquitocoin Core
...
When enabled, this option tells Quiquitocoin Core that it is running in an
...
IPv6 local network. This helps Quiquitocoin Core perform better address management:
...
CJDNS support was added to Quiquitocoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `quiquitocoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Quiquitocoin Core

There are several ways to see your CJDNS address in Quiquitocoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `quiquitocoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `quiquitocoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `quiquitocoin-cli help getnodeaddresses` for details.
