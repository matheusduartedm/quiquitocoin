# PSBT Howto for Quiquitocoin Core

Since Quiquitocoin Core 0.17, an RPC interface exists for Partially Signed Quiquitocoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/quiquitocoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Quiquitocoin transactions that are not fully signed
...
#### Multisig with multiple Quiquitocoin Core instances
...
Quiquitocoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `quiquitocoin-cli` is used.
