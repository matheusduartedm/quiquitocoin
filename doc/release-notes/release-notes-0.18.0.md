Quiquitocoin Core version 0.18.0 is now available from:

  <https://quiquitocoincore.org/bin/quiquitocoin-core-0.18.0/>
...
`/Applications/Quiquitocoin-Qt` (on Mac) or `quiquitocoind`/`quiquitocoin-qt` (on
Linux).
...
recommended to use Quiquitocoin Core on unsupported systems.

Quiquitocoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Quiquitocoin Core does not yet change appearance when
...
`contrib/init/quiquitocoind.service`, it has been changed to use
`/var/lib/quiquitocoind` as the data directory instead of
`~quiquitocoin/.quiquitocoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/quiquitocoind` will
exist has enough space (check using `df -h /var/lib/quiquitocoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://quiquitocoincore.org/en/contact/) the Quiquitocoin Core
...
The systemd init file (`contrib/init/quiquitocoind.service`) has been changed
to use `/var/lib/quiquitocoind` as the data directory instead of
`~quiquitocoin/.quiquitocoin`. This change makes Quiquitocoin Core more consistent with
...
`datadir` option in `/etc/quiquitocoin/quiquitocoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/quiquitocoin/quiquitocoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/quiquitocoin-conf.md)
  about the `quiquitocoin.conf` file describes how to use it to configure
  Quiquitocoin Core.

- A new document introduces Quiquitocoin Core's BIP174 [Partially-Signed
  Quiquitocoin Transactions
  (PSBT)](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Quiquitocoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Quiquitocoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/quiquitocoin/quiquitocoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Quiquitocoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Quiquitocoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/quiquitocoin/quiquitocoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart quiquitocoind with the `-deprecatedrpc=generate`
...
- A new `quiquitocoin-wallet` tool is now distributed alongside Quiquitocoin
...
This section describes planned changes to Quiquitocoin Core that may affect
other Quiquitocoin software and services.

- Since version 0.16.0, Quiquitocoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Quiquitocoin Core 0.20 (expected about
  a year after 0.18), Quiquitocoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Quiquitocoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Quiquitocoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Quiquitocoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Quiquitocoin Core
...
- A sub-project of Quiquitocoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Quiquitocoin Core.  See their [project
  page](https://github.com/quiquitocoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Quiquitocoin Core's own implementation, although entropy
  gathered by Quiquitocoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Quiquitocoin Core a
...
- On macOS, Quiquitocoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `quiquitocoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libquiquitocoin_server.a` (jnewbery)
...
- #13926 [Tools] quiquitocoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like quiquitocoind (MarcoFalke)
...
- #14521 qt, docs: Fix `quiquitocoin-qt -version` output formatting (hebasto)
...
- #11625 Add QuiquitocoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libquiquitocoin_server` (laanwj)
...
- #15154 Configure: quiquitocoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/quiquitocoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Quiquitocoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libquiquitocoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in quiquitocoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Quiquitocoin Core (Christewart)
...
- #12246 Bugfix: Only run quiquitocoin-tx tests when quiquitocoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_quiquitocoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_quiquitocoin` (MarcoFalke)
...
- #14186 quiquitocoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_quiquitocoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update quiquitocoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/quiquitocoin-conf.md (hebasto)
...
- #15354 Add missing `quiquitocoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct quiquitocoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support quiquitocoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/quiquitocoin/).
