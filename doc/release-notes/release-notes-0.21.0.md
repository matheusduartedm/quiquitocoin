...
Quiquitocoin Core version 0.21.0 is now available from:

  <https://quiquitocoincore.org/bin/quiquitocoin-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/quiquitocoin/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Quiquitocoin Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Quiquitocoin Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/quiquitocoin/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/quiquitocoin/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/quiquitocoin/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/quiquitocoin/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/quiquitocoin/bips/blob/master/bip-0342.mediawiki)),
...
- A new `quiquitocoin-cli -netinfo` command provides a network peer connections
...
- A new `quiquitocoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `quiquitocoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `quiquitocoin-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Quiquitocoin Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `quiquitocoin-wallet` tool which performs the salvage
...
Quiquitocoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`quiquitocoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Quiquitocoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `quiquitocoin-cli createwallet` or `quiquitocoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/quiquitocoin/quiquitocoin/issues).
...
Users who already have a Quiquitocoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Quiquitocoin Core.
...
The change to using descriptors changes the default derivation paths used by Quiquitocoin Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Quiquitocoin Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `quiquitocoin.conf`
...
- #19284 Add seed.quiquitocoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in quiquitocoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add quiquitocoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in quiquitocoin-cli (laanwj)
...
### Build system
- #18504 Drop quiquitocoin-tx and quiquitocoin-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in QUIQUITOCOIN_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for quiquitocoin-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Quiquitocoin Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrequiquitocoin)
...
- #18817 Document differences in quiquitocoind and quiquitocoin-qt locale handling (practicalswift)
