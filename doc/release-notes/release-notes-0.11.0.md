Quiquitocoin Core version 0.11.0 is now available from:

  <https://quiquitocoin.org/bin/quiquitocoin-core-0.11.0/>
...
For example, add the following to `quiquitocoin.conf`:
...
related to the blockchain in the quiquitocoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Quiquitocoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Quiquitocoin Core will 
...
For an example script see [quiquitocoin-submittx](https://github.com/laanwj/quiquitocoin-submittx).
...
- #5334 `cf87536` libquiquitocoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new quiquitocoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only QuiquitocoinAmountField
- #5753 `0f44672` Add quiquitocoin logo to about screen
...
- #5809 `46bfbe7` Add quiquitocoin-cli man page
...
- #5849 `41259ca` contrib/init/quiquitocoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Quiquitocoin-Qt.app to Quiquitocoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for quiquitocoin-tx)
