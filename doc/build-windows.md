...
Below are some notes on how to build Quiquitocoin Core for Windows.

The options known to work for building Quiquitocoin Core on Windows are:
...
Note that for WSL the Quiquitocoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/quiquitocoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\quiquitocoin`, for example:

    make install DESTDIR=/mnt/c/workspace/quiquitocoin
