# User-space, Statically Defined Tracing (USDT) for Quiquitocoin Core

Quiquitocoin Core includes statically defined tracepoints to allow for more
...
**Note**: Quiquitocoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Quiquitocoin Core
...
Multiple tools can list the available tracepoints in a `quiquitocoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Quiquitocoin Core, use `info probes` in `gdb`:

```
$ gdb ./src/quiquitocoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/quiquitocoind
stap net        outbound_message 0x0000000000107c05 /src/quiquitocoind
stap validation block_connected  0x00000000002fb10c /src/quiquitocoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Quiquitocoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/quiquitocoind | grep NT_STAPSDT -A 4 -B 2
