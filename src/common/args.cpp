...
const char * const QUIQUITOCOIN_CONF_FILENAME = "quiquitocoin.conf";
const char * const QUIQUITOCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //quiquitocoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", QUIQUITOCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Quiquitocoin
    // macOS: ~/Library/Application Support/Quiquitocoin
    // Unix-like: ~/.quiquitocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Quiquitocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Quiquitocoin";
#else
    // Unix-like
    return pathRet / ".quiquitocoin";
#endif
#endif
}
