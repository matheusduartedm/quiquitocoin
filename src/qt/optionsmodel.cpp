...
/** Call node.updateRwSetting() with Quiquitocoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Quiquitocoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/quiquitocoin/quiquitocoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because quiquitocoin 22.x will not try to read these.
...
//! Convert enabled/size values to quiquitocoin -prune setting.
...
//! Get pruning enabled value to show in GUI from quiquitocoin -prune setting.
...
//! Get pruning size value to show in GUI from quiquitocoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayQuiquitocoinUnit")) {
        settings.setValue("DisplayQuiquitocoinUnit", QVariant::fromValue(QuiquitocoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayQuiquitocoinUnit");
    if (unit.canConvert<QuiquitocoinUnit>()) {
        m_display_quiquitocoin_unit = unit.value<QuiquitocoinUnit>();
    } else {
        m_display_quiquitocoin_unit = QuiquitocoinUnit::BTC;
        settings.setValue("DisplayQuiquitocoinUnit", QVariant::fromValue(m_display_quiquitocoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if quiquitocoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_quiquitocoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<QuiquitocoinUnit>() == m_display_quiquitocoin_unit) return;
    m_display_quiquitocoin_unit = new_unit.value<QuiquitocoinUnit>();
    QSettings settings;
    settings.setValue("DisplayQuiquitocoinUnit", QVariant::fromValue(m_display_quiquitocoin_unit));
    Q_EMIT displayUnitChanged(m_display_quiquitocoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/quiquitocoin/quiquitocoin/pull/8273
    ...
    // (https://github.com/quiquitocoin-core/gui/issues/567).
