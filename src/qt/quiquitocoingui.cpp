...
#include <qt/quiquitocoingui.h>

#include <qt/quiquitocoinunits.h>
...
const std::string QuiquitocoinGUI::DEFAULT_UIPLATFORM =
...
QuiquitocoinGUI::QuiquitocoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &QuiquitocoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &QuiquitocoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &QuiquitocoinGUI::showModalOverlay);
...
QuiquitocoinGUI::~QuiquitocoinGUI()
{
...
void QuiquitocoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Quiquitocoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and quiquitocoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &QuiquitocoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &QuiquitocoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &QuiquitocoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Quiquitocoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Quiquitocoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Quiquitocoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Quiquitocoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a quiquitocoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Quiquitocoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &QuiquitocoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &QuiquitocoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &QuiquitocoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &QuiquitocoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &QuiquitocoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &QuiquitocoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &QuiquitocoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &QuiquitocoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &QuiquitocoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &QuiquitocoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &QuiquitocoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &QuiquitocoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &QuiquitocoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &QuiquitocoinGUI::showDebugWindow);
}

void QuiquitocoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void QuiquitocoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &QuiquitocoinGUI::setCurrentWalletBySelectorIndex);
...
void QuiquitocoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &QuiquitocoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &QuiquitocoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &QuiquitocoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &QuiquitocoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void QuiquitocoinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void QuiquitocoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &QuiquitocoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &QuiquitocoinGUI::removeWallet);
...
WalletController* QuiquitocoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void QuiquitocoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &QuiquitocoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &QuiquitocoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &QuiquitocoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &QuiquitocoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &QuiquitocoinGUI::incomingTransaction);
    connect(this, &QuiquitocoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void QuiquitocoinGUI::removeWallet(WalletModel* walletModel)
{
...
void QuiquitocoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void QuiquitocoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void QuiquitocoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void QuiquitocoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void QuiquitocoinGUI::createTrayIcon()
{
...
void QuiquitocoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &QuiquitocoinGUI::toggleHidden);
...
void QuiquitocoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void QuiquitocoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void QuiquitocoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void QuiquitocoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void QuiquitocoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void QuiquitocoinGUI::openClicked()
{
...
void QuiquitocoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void QuiquitocoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void QuiquitocoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void QuiquitocoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void QuiquitocoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void QuiquitocoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void QuiquitocoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void QuiquitocoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Quiquitocoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void QuiquitocoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void QuiquitocoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void QuiquitocoinGUI::updateHeadersSyncProgressLabel()
{
...
void QuiquitocoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void QuiquitocoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &QuiquitocoinGUI::quitRequested);
...
void QuiquitocoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void QuiquitocoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &QuiquitocoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void QuiquitocoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void QuiquitocoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &QuiquitocoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &QuiquitocoinGUI::show);
                e->ignore();
            }
        }
    }
...
void QuiquitocoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void QuiquitocoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void QuiquitocoinGUI::incomingTransaction(const QString& date, QuiquitocoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(QuiquitocoinUnits::formatWithUnit(unit, amount, true));
...
void QuiquitocoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void QuiquitocoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool QuiquitocoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool QuiquitocoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void QuiquitocoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void QuiquitocoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void QuiquitocoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void QuiquitocoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void QuiquitocoinGUI::updateWindowTitle()
{
...
void QuiquitocoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void QuiquitocoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void QuiquitocoinGUI::detectShutdown()
{
...
void QuiquitocoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void QuiquitocoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(QuiquitocoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = QuiquitocoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void QuiquitocoinGUI::subscribeToCoreSignals()
{
...
void QuiquitocoinGUI::unsubscribeFromCoreSignals()
{
...
bool QuiquitocoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<QuiquitocoinUnit> units = QuiquitocoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const QuiquitocoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, QuiquitocoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const QuiquitocoinUnit u : QuiquitocoinUnits::availableUnits()) {
        menu->addAction(QuiquitocoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(QuiquitocoinUnit newUnits)
{
    setText(QuiquitocoinUnits::longName(newUnits));
}
