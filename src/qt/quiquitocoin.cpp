...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<QuiquitocoinUnit>("QuiquitocoinUnit");
#else
    qRegisterMetaType<QuiquitocoinUnit>("QuiquitocoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. quiquitocoin_de.qm (shortcut "de" needs to be defined in quiquitocoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. quiquitocoin_de_DE.qm (shortcut "de_DE" needs to be defined in quiquitocoin.qrc)
...
static const char* qt_argv = "quiquitocoin-qt";

QuiquitocoinApplication::QuiquitocoinApplication()
...
void QuiquitocoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the QuiquitocoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", QuiquitocoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void QuiquitocoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool QuiquitocoinApplication::createOptionsModel(bool resetSettings)
{
...
void QuiquitocoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new QuiquitocoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &QuiquitocoinGUI::quitRequested, this, &QuiquitocoinApplication::requestShutdown);
...
void QuiquitocoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void QuiquitocoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool QuiquitocoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void QuiquitocoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &QuiquitocoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &QuiquitocoinApplication::handleRunawayException);
    connect(this, &QuiquitocoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &QuiquitocoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void QuiquitocoinApplication::parameterSetup()
{
...
void QuiquitocoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void QuiquitocoinApplication::requestInitialize()
{
...
void QuiquitocoinApplication::requestShutdown()
{
...
void QuiquitocoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // quiquitocoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &QuiquitocoinGUI::handlePaymentRequest);
            connect(window, &QuiquitocoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void QuiquitocoinApplication::handleRunawayException(const QString &message)
{
...
void QuiquitocoinApplication::handleNonFatalException(const QString& message)
{
...
WId QuiquitocoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool QuiquitocoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", QuiquitocoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(quiquitocoin);
    Q_INIT_RESOURCE(quiquitocoin_locale);
    ...
    /// 6-7. Parse quiquitocoin.conf, determine network, switch to network specific
    ...
    // quiquitocoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
