This directory contains the source code for the Quiquitocoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### quiquitocoingui.(h/cpp)

- Represents the main window of the Quiquitocoin UI.
...
- ClientModel is used by the main application `quiquitocoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a quiquitocoin:... link from a browser).
...
* UI elements like QuiquitocoinAmountField, which inherit from QWidget.
* `quiquitocoinstrings.cpp`: automatically generated
* `quiquitocoinunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/quiquitocoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `quiquitocoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `quiquitocoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `quiquitocoin-qt`
9. You're all set! Start developing, building, and debugging the Quiquitocoin Core GUI
