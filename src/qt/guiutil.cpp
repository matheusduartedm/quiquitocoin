...
#include <qt/quiquitocoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Quiquitocoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new QuiquitocoinAddressEntryValidator(parent));
    widget->setCheckValidator(new QuiquitocoinAddressCheckValidator(parent));
}
...
bool parseQuiquitocoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no quiquitocoin: URI
    if(!uri.isValid() || uri.scheme() != QString("quiquitocoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!QuiquitocoinUnits::parse(QuiquitocoinUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseQuiquitocoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseQuiquitocoinURI(uriInstance, out);
}

QString formatQuiquitocoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("quiquitocoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(QuiquitocoinUnits::format(QuiquitocoinUnit::BTC, info.amount, false, QuiquitocoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openQuiquitocoinConf()
{
    ...
    /* Open quiquitocoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Quiquitocoin.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Quiquitocoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Quiquitocoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Quiquitocoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "quiquitocoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("quiquitocoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a quiquitocoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Quiquitocoin\n";
        else
            optionFile << strprintf("Name=Quiquitocoin (%s)\n", ChainTypeToString(chain));
