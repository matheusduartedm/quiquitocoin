...
{
    // If Qt is used, parameters/quiquitocoin.conf are parsed in qt/quiquitocoin.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see quiquitocoind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  quiquitocoind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for quiquitocoind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect quiquitocoind signal handlers
    noui_connect();
