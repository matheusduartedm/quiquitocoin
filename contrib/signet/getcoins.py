...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing quiquitocoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='quiquitocoin-cli', help='quiquitocoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Quiquitocoin address to which the faucet should send')
...
parser.add_argument('quiquitocoin_cli_args', nargs='*', help='Arguments to pass on to quiquitocoin-cli (default: -signet)')

args = parser.parse_args()

if args.quiquitocoin_cli_args == []:
    args.quiquitocoin_cli_args = ['-signet']


def quiquitocoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.quiquitocoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = quiquitocoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = quiquitocoin_cli(['getnewaddress', 'faucet', 'bech32'])
