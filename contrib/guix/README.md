# Bootstrappable Quiquitocoin Core Builds

This directory contains the files necessary to perform bootstrappable Quiquitocoin
Core builds.
...
To build Quiquitocoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Quiquitocoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/quiquitocoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/quiquitocoin-core/guix.sigs).
