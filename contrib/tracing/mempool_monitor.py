...
""" Example logging Quiquitocoin Core mempool events using the mempool:added,
...
def main(quiquitocoind_path):
    quiquitocoind_with_usdts = USDT(path=str(quiquitocoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    quiquitocoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    quiquitocoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    quiquitocoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    quiquitocoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[quiquitocoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/quiquitocoind")
        exit(1)
