...
"""Script for verifying Quiquitocoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from quiquitocoincore.org and quiquitocoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/quiquitocoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://quiquitocoincore.org"
HOST2 = "https://quiquitocoin.org"
VERSIONPREFIX = "quiquitocoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Quiquitocoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"quiquitocoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by quiquitocoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the quiquitocoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes quiquitocoin.org lags behind quiquitocoincore.org.)')
    )
