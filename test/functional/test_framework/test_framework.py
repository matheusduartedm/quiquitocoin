...
TMPDIR_PREFIX = "quiquitocoin_func_test_"
...
class QuiquitocoinTestMetaClass(type):
    """Metaclass for QuiquitocoinTestFramework.

    Ensures that any attempt to register a subclass of `QuiquitocoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'QuiquitocoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("QuiquitocoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("QuiquitocoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class QuiquitocoinTestFramework(metaclass=QuiquitocoinTestMetaClass):
    """Base class for a quiquitocoin test script.

    Individual quiquitocoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave quiquitocoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop quiquitocoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use quiquitocoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "quiquitocoind": ("quiquitocoind", "QUIQUITOCOIND"),
            "quiquitocoin-cli": ("quiquitocoincli", "QUIQUITOCOINCLI"),
            "quiquitocoin-util": ("quiquitocoinutil", "QUIQUITOCOINUTIL"),
            "quiquitocoin-wallet": ("quiquitocoinwallet", "QUIQUITOCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: quiquitocoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("QuiquitocoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'quiquitocoind', self.options.quiquitocoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'quiquitocoin-cli', self.options.quiquitocoincli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            test_node_i = TestNode(
                ...
                quiquitocoind=binary[i],
                quiquitocoin_cli=binary_cli[i],
                version=versions[i],
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a quiquitocoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple quiquitocoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as quiquitocoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    quiquitocoind=self.options.quiquitocoind,
                    quiquitocoin_cli=self.options.quiquitocoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in quiquitocoin.conf
    ...
    def skip_if_no_quiquitocoind_tracepoints(self):
        """Skip the running test if quiquitocoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("quiquitocoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_quiquitocoind_zmq(self):
        """Skip the running test if quiquitocoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("quiquitocoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if quiquitocoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("quiquitocoin-wallet has not been compiled")

    def skip_if_no_quiquitocoin_util(self):
        """Skip the running test if quiquitocoin-util has not been compiled."""
        if not self.is_quiquitocoin_util_compiled():
            raise SkipTest("quiquitocoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if quiquitocoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("quiquitocoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether quiquitocoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether quiquitocoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_quiquitocoin_util_compiled(self):
        """Checks whether quiquitocoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_QUIQUITOCOIN_UTIL")
